...
async def test_exclusions_are_explained_not_just_listed(router):
    ...
    for e in ex["excluded"]:
        assert e["plain"]
        assert e["plain"] != e["reason"] or "circuit" not in e["reason"]
    assert any("not capable enough" in e["plain"] for e in ex["excluded"])
